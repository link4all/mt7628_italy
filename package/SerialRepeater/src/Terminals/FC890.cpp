...
static const unsigned char FC890CMD_READALL[][13] =
{
    {0x0D, 0x0A, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x24, 0x00, 0x00},
    {0x0D, 0x0A, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x14, 0x00, 0x00},
    {0x0D, 0x0A, 0x00, 0x00, 0x00, 0x00, 0x03, 0x03, 0x00, 0x00, 0x3C, 0x00, 0x00}
};
...
{
    int cmdlen = sizeof(FC890CMD_READALL[cmdid]);
    int payload_len = cmdlen - 2;

    if (!id || !cmdbuf || buflen < cmdlen) return 0;

    memset(cmdbuf, 0, buflen);
    memcpy(cmdbuf, FC890CMD_READALL[cmdid], cmdlen);
...
void FC890_Update(const char *id, const unsigned char *raw, int type, bool submit, TerminalDataSet &dataset, Document *Json)
{
    ...
    if (it != dataset.end()) {
        ...
        switch (type) {
        case FC890_CMD_READALL_STATUS:
            memcpy((void *)&ds.Data.FC890.Status, (unsigned char *)&raw[8], sizeof(ds.Data.FC890.Status));
            break;
        case FC890_CMD_READALL_COUNTER:
            memcpy((void *)&ds.Data.FC890.Counter, (unsigned char *)&raw[8], sizeof(ds.Data.FC890.Counter));
            break;
        case FC890_CMD_READALL_SETTINGS:
            memcpy((void *)&ds.Data.FC890.Settings, (unsigned char *)&raw[8], sizeof(ds.Data.FC890.Settings));
            break;
        }
